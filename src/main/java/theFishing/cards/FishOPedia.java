...
public class FishOPedia extends AbstractFishingCard {
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        ...
        if (fishes.isEmpty()) {
            atb(new TalkAction(true, "Complete!", 0.75F, 2.0F));
            applyToSelf(new StrengthPower(p, 4));
        } else {
            atb(new TalkAction(true, "left: " + fishes.size(), 0.75F, 2.0F));
            AbstractCard q = CardLibrary.getCard(fishes.get(AbstractDungeon.cardRandomRng.random(fishes.size() - 1)));
