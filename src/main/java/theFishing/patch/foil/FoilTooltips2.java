...
public class FoilTooltips2 {
    ...
    public static void InsertAfter(SingleCardViewPopup __instance, SpriteBatch sb, AbstractCard acard, @ByRef ArrayList<PowerTip>[] t) {
        ...
        if (FoilPatches.isFoil(acard) && !acard.rawDescription.toLowerCase().contains(FoilTooltips1.UI_STRINGS.TEXT[0])) {
            tooltips.add(new TooltipInfo(BaseMod.getKeywordTitle(FoilTooltips1.UI_STRINGS.TEXT[0]), BaseMod.getKeywordDescription(FoilTooltips1.UI_STRINGS.TEXT[0])));
        }
        if (!tooltips.isEmpty())
